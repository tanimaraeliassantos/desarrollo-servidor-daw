...
public class MercanciaServiceImpl implements IMercanciaService {
	...
	//Añadido apra prueba trimestral
	@Override
	public Map<String, Long> obtenerEstadisticasPorEstado() {
		long pendientes = repo.findByEstado("pendiente").size();
		long asignadas = repo.findByEstado("asignada").size();
		long entregadas = repo.findByEstado("entregada").size();
	...
	//Añadido apra prueba trimestral
	@Override
	public List<Mercancia> buscarPorDistanciaMinima(Double distanciaMinima) {
		if(distanciaMinima == null) {
			return repo.findAll();
		}
		return repo.findByDistanciaKmGreaterThan(distanciaMinima);
	}
}
